...
{
    public class MutableBulkheadSpecsHelper : IDisposable
    {
        protected ITestOutputHelper testOutputHelper;

        protected TimeSpan shimTimeSpan = TimeSpan.FromMilliseconds(2000); // Consider increasing shimTimeSpan if MutableBulkhead specs fail transiently in slower build environments.

        internal TraceableAction[] actions;

        protected readonly AutoResetEvent statusChanged = new AutoResetEvent(false);

        public MutableBulkheadSpecsHelper(ITestOutputHelper testOutputHelper)
        {
#if !DEBUG 
        ...
        /// <summary>
        /// Asserts that the actionContainingAssertions will succeed without <see cref="AssertionFailedException"/> or <see cref="XunitException"/>, within the given timespan.  Checks are made each time a status-change pulse is received from the <see cref="TraceableAction"/>s executing through the MutableBulkhead.
        /// </summary>
