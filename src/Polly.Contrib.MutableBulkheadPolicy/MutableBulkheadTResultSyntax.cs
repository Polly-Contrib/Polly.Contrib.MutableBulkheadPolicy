...
namespace Polly.Contrib.MutableBulkheadPolicy
{
    ...
    {
        /// <summary>
        /// <para>Builds a mutable bulkhead isolation <see cref="Policy{TResult}"/>, which limits the maximum concurrency of actions executed through the policy.  Imposing a maximum concurrency limits the potential of governed actions, when faulting, to bring down the system.</para>
        ...
        public static MutableBulkheadPolicy<TResult> Create<TResult>(int maxParallelization)
        {
            Action<Context> doNothing = _ => { };
            return Create<TResult>(maxParallelization, 0, doNothing);
        }
        ...
        /// <returns>The policy instance.</returns>
        public static MutableBulkheadPolicy<TResult> Create<TResult>(int maxParallelization, Action<Context> onBulkheadRejected)
            => Create<TResult>(maxParallelization, 0, onBulkheadRejected);
        ...
        public static MutableBulkheadPolicy<TResult> Create<TResult>(int maxParallelization, int maxQueuingActions)
        {
            Action<Context> doNothing = _ => { };
            return Create<TResult>(maxParallelization, maxQueuingActions, doNothing);
        }
